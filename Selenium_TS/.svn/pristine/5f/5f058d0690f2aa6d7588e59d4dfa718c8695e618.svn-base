...
public class TSSystemUI {
	...
	/**
	 * precondition: switch to ndadmin
	 * in EKPTestData filed "ScheduleTask", just use ":" to separate frequency and type
	 * if have more than one schedule task need to run, separate by line
	 * example-
	 * Weekly:All
	 * Daily:All
	 * @param driver
	...
	public static void runScheduledTask(WebDriver driver, String task) {
		Navigator.navigate(driver, Navigator.xmlWebElmtMgr.getNavigationPathList(
				"ManageCenter", "ScheduledTasks"));
		
		String frequency = "", type = "";
		HashMap<String, ArrayList<String>> tasks = CriteriaParser.parseKeyValueList(":", "", task);
		...
		while (keySetIterator.hasNext()){
			...
			JUnitAssert.assertTrue(WebDriverUtils.textPresentInPage(driver,Labels.Msg_Tasks_Completed.toString()),
	...
	/**
	 * precondition: switch to ndadmin
	 * in EKPTestData field "SysConf", just use ":" to separate sys config type and value
	 * if have more than one sys config need to set, separate by line
	 * example-
	 * Enable Session-level Catalog Search Results:disable
	 * Search result format.:Learning Module/Program Name, Learning Type, Catalog, and Brief Description
	 * @param driver
	 * @param conf
	 */
	public static void setupSystemConf(WebDriver driver, String conf){
		String key = "", value = "";
		HashMap<String, ArrayList<String>> confs = CriteriaParser.parseKeyValueList(":", "", conf);
		Navigator.navigate(driver, Navigator.xmlWebElmtMgr.getNavigationPathList(
				"ManageCenter", "SystemConf"));
		...
		while (keySetIterator.hasNext()){
			...
			by = By.xpath("//tr[descendant::td/div[contains(text(),'"+key+"')]]/td/div[descendant::label[contains(text(),'"+value+"')]]/input");
