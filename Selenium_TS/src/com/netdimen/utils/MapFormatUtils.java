 ...
 * 
 * <pre>
 * MapFormatUtils.format(&quot;Hello {name}&quot;, map);
 * </pre>
 * 
...
public class MapFormatUtils extends Format {
	...
	public String processPattern(String newPattern)
		...
		while (true) {
			...
			/*
			 * Range ran = skipped.getRangeContainingOffset(lidx); if (ran !=
			 * null) { outpat.append(newPattern.substring(idx, ran.getEnd()));
			...
			arguments[offnum] = newPattern
					.substring(lidx + ldel.length(), ridx);
	...
	private String formatObject(Object obj) {
		...
		if (obj instanceof Number) {
			...
			return DateFormat.getDateTimeInstance(DateFormat.SHORT,
					DateFormat.SHORT, locale).format(obj); // fix
	 ...
	 * @return Formatted string
	 */
	public StringBuffer format(Object pat, StringBuffer result,
			FieldPosition fpos) {
		...
		for (int i = 0; i <= maxOffset; ++i) {
			...
			if (obj == null) {
				// try less-greedy match; useful for e.g. PROP___PROPNAME__
				// where
				// 'PROPNAME' is a key and delims are both '__'
				// this does not solve all possible cases, surely, but it should
				// catch
				...
				if (fromright > 0) {
					String newkey = lessgreedy.substring(fromright
							+ ldel.length());
		...
		while (key_it.hasNext()) {
			...
			do {
				...
				if (it_idx >= 0 /* && !skipped.containsOffset(it_idx) */) {
					sbuf.replace(it_idx, it_idx + it_obj.length(), ldel
							+ it_key + rdel);
