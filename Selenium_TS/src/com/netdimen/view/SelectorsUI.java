...
public class SelectorsUI {
	 ...
	 * @param keywords
	 */
	private static void general_Selector(WebDriver driver, String pagename, ArrayList<String> keywords){
		if(keywords.size() > 0){
			By by = null;
			//WebElementWrapper searchElm = null;
			for (String keyword : keywords) {
				by=XMLWebElementManager.getInstance().getBy(pagename, "SearchText");
				WebDriverUtils.fillin_textbox(driver, by, keyword);
				
			
				by=XMLWebElementManager.getInstance().getBy(pagename, "SearchButton");
				...
				by=XMLWebElementManager.getInstance().getBy(pagename, "AddSingle");
				WebDriverUtils.clickButton(driver, by);
			}
	 ...
	 * @param keywords
	 */
	private static void UserIDValidator(WebDriver driver, String pagename, ArrayList<String> keywords){
		if(keywords.size() > 0){
			By by = null;
			
			for (String keyword : keywords) {
				
				by=XMLWebElementManager.getInstance().getBy( pagename, "DataEntryList");
				WebDriverUtils.fillin_textbox(driver, by, keyword);
				
				
				by=XMLWebElementManager.getInstance().getBy(pagename, "ValidatorBtn");
				WebDriverUtils.clickButton(driver, by);
			}
